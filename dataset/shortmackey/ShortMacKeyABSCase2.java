...
class ShortMacKey2 {
    ...
    public ShortMacKey2(byte[] defaultKeyBytes) {
        keyBytes = defaultKeyBytes;
    }

    public void go(byte[] passedKeyBytes) throws NoSuchAlgorithmException, NoSuchPaddingException, InvalidKeyException, IllegalBlockSizeException, BadPaddingException, InvalidAlgorithmParameterException, InvalidKeySpecException {
        passedKeyBytes = keyBytes;
        ...
        SecretKeySpec keySpec = new SecretKeySpec(passedKeyBytes, "HmacSHA1");
...
public class ShortMacKeyABSCase2 {
    public static void main(String[] args) throws NoSuchPaddingException, NoSuchAlgorithmException, InvalidKeyException, IOException, IllegalBlockSizeException, BadPaddingException, ClassNotFoundException, InvalidAlgorithmParameterException, InvalidKeySpecException {
        byte[] defaultKeyBytes = new byte[15];
        SecureRandom random1 = new SecureRandom();
        random1.nextBytes(defaultKeyBytes);

        byte[] passedKeyBytes = new byte[16];
        SecureRandom random2 = new SecureRandom();
        random2.nextBytes(defaultKeyBytes);

        ShortMacKey2 case2 = new ShortMacKey2(defaultKeyBytes);
        case2.go(passedKeyBytes);
    }
}