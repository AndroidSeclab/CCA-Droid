...
`targetSchemeType` : The authenticated encryption type(e.g., Encrypt-then-MAC) to find in the slices
...
`targetConstant` : The constant using regular expressions to find in the slices
...
2. To start the analysis, open a terminal and enter the following command:
    ```bash
    java -jar ./CCA-Droid.jar ./sample.apk ./rules >> result.txt
    ```
